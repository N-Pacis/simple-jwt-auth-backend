...
describe("Product module test", () => {
  ...
  describe("Register product feature test", () => {
    test("Should return 200 success response code on successful product registration", async () => {
  ...
  describe('Get products feature test', () => { 
    test("Should return 200 success response code and an array of products", async () => {
      const response = await request
        .get("/products")
        .set("auth-token", `Bearer ${auth_token}`);
  ...
  describe('Update product feature test', () => {
    ...
    test("Should return 404 error response code when the product does not exist", async () => {
      const response = await request.put("/products/update/1000").send({
        name: "Updated Product",
        description: "This product has been updated",
        price: 39.99,
      })
  ...
  describe('Delete product feature test', () => {
    test("Should return 200 success response code on successful product deletion", async () => {
      const existingProduct = await ProductModel.create({
        name: "Existing Product",
        description: "This is an existing product",
        price: 29,
    ...
    test("Should return 404 error response code when the product does not exist", async () => {
      const response = await request.delete("/products/delete/1000")
      .set("auth-token", `Bearer ${auth_token}`);
